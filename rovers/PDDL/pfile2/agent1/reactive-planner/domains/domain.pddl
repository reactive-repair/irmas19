...
(:action comm_rover_soil_data
 :parameters (?r - rover ?r2 - rover ?p - waypoint ?x - waypoint)
 :precondition (and (= (at ?r) ?x) 
		(= (equipped_for_com_rover_to_rover ?r) Yes)
		(= (have_soil_analysis ?r ?p) Yes) )
 :effect (and
		(assign (have_soil_analysis ?r ?p) Not) (assign (have_soil_analysis ?r2 ?p) Yes)))
...
(:action comm_rover_rock_data
 ...
 :precondition (and (= (at ?r) ?x)
		(= (equipped_for_com_rover_to_rover ?r) Yes)
		(= (have_rock_analysis ?r ?p) Yes))
 :effect (and
		(assign (have_rock_analysis ?r ?p) Not) (assign (have_rock_analysis ?r2 ?p) Yes)))

(:action comm_rover_image_data
 :parameters (?r - rover ?r2 - rover ?o - objective ?m - mode ?x - waypoint)
 :precondition (and	(= (at ?r) ?x)
		(= (equipped_for_com_rover_to_rover ?r) Yes)
		(= (have_image ?r ?o ?m) Yes))
 :effect (and
 		(assign (have_image ?r ?o ?m) Not) (assign (have_image ?r2 ?o ?m) Yes)))
